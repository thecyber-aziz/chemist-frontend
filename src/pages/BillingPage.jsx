      ...
      <div className="max-w-7xl mx-auto space-y-8 pb-24 md:pb-8">
        <div className="pt-4 pb-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">Billing</h1>
          </div>
        </div>

        <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="mb-6 flex flex-row flex-wrap items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-blue-100 px-4 py-2 text-sm font-bold text-blue-700 border border-blue-200">
                {cartProducts.length} item{cartProducts.length === 1 ? '' : 's'}
              </span>
            </div>

            {cartProducts.length === 0 ? (
              <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-8 text-center text-slate-500 sm:p-10">
                <p className="font-semibold">No items in cart.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {cartProducts.map((item) => (
                  <div key={item.id} className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4 hover:border-slate-300 hover:shadow-sm transition-all duration-300">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="h-16 w-16 overflow-hidden rounded-xl bg-slate-100 shadow-sm border border-slate-200 sm:h-18 sm:w-18">
                        {item.imageUrl ? (
                          <img
                            src={item.imageUrl}
                          ...
                          <div className="flex h-full items-center justify-center text-slate-400 text-xs font-semibold">No Image</div>
                        )}
                      </div>
                      <div className="min-w-0">
                        <p className="text-base font-bold text-slate-900">{item.medicineName}</p>
                        <p className="text-sm text-slate-600 font-semibold">₹{item.sellingPrice}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center rounded-full border border-slate-200 bg-white shadow-sm">
                        <button
                          ...
                          className="rounded-l-full px-3 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-100 transition-colors"
                        >
                          <Minus className="h-4 w-4" />
                        </button>
                        <div className="px-4 text-sm font-bold text-slate-800">{item.quantity}</div>
                        <button
                          type="button"
                          onClick={() => updateCartQuantity(item.id, item.quantity + 1)}
                          className="rounded-r-full px-3 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-100 transition-colors"
                        >
                      ...
                      <button
                        ...
                        className="rounded-full bg-red-100 px-3.5 py-2.5 text-sm font-bold text-red-600 hover:bg-red-200 transition-colors border border-red-200"
                      >
          ...
          <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-blue-50 p-6 sm:p-8 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="mb-8 flex items-center justify-between">
              <p className="text-lg font-bold text-slate-700 uppercase tracking-wider">Total Amount</p>
            ...
            <div className="space-y-5">
              <div>
                <label className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-700 mb-3 ml-1">
                  <User className="h-4 w-4 text-blue-600" /> Customer Name
                </label>
                <input
                  type="text"
                  value={customerName}
                  onChange={(e) => setCustomerName(e.target.value)}
                  className="w-full rounded-xl border border-slate-200 bg-white px-5 py-3.5 text-sm text-slate-800 font-semibold shadow-sm hover:border-slate-300 focus:border-slate-400 focus:ring-1 focus:ring-slate-200 transition duration-300 placeholder:text-slate-400"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="3"
                  className="w-full rounded-xl border border-slate-200 bg-white px-5 py-3.5 text-sm text-slate-800 font-semibold shadow-sm hover:border-slate-300 focus:border-slate-400 focus:ring-1 focus:ring-slate-200 transition duration-300 placeholder:text-slate-400 resize-none"
                  placeholder="Add description (optional)"
                />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-slate-700 mb-3">Payment Method</p>
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setPaymentMethod('Cash')}
                    className={`flex-1 rounded-xl border px-5 py-3.5 text-sm font-bold transition duration-300 ${
                      paymentMethod === 'Cash'
                        ? 'border-blue-300 bg-blue-100 text-blue-700 shadow-md'
                        : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50'
                    }`}
                  >
                    <span className="inline-flex items-center justify-center gap-2"><Wallet className="h-5 w-5" /> Cash</span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    <span className="inline-flex items-center justify-center gap-2"><Smartphone className="h-5 w-5" /> Online / UPI</span>
                  </button>
                </div>
              </div>
              <button
                ...
                className="mt-2 w-full rounded-xl bg-slate-900 px-6 py-4 text-sm font-bold text-white shadow-lg hover:bg-slate-800 hover:shadow-xl transition-all duration-300"
              >
                Confirm Order
              </button>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-10 text-center text-slate-500 font-semibold">
            ...
            <div className="overflow-x-auto -mx-6 px-6 sm:mx-0 sm:px-0">
              <table className="min-w-[900px] w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Sr</th>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Order ID</th>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Customer</th>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Phone</th>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Amount</th>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Payment</th>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Date</th>
                    <th className="whitespace-nowrap px-4 py-4 text-left font-bold text-slate-700 text-xs uppercase tracking-widest">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 bg-white">
                  {filteredOrders.map((order, index) => (
                    ...
                    >
                      <td className="whitespace-nowrap px-4 py-4 text-slate-700 font-semibold">{index + 1}</td>
                      <td className="whitespace-nowrap px-4 py-4 font-mono text-xs text-slate-600 font-bold">{order.orderId}</td>
                      <td className="whitespace-nowrap px-4 py-4 text-slate-700">
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-100 text-blue-600 border border-blue-200 font-bold">
                            <User className="h-5 w-5" />
                          </div>
                          <span className="font-semibold">{order.customerName}</span>
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-4 py-4 text-slate-700 font-semibold">{order.phoneNumber}</td>
                      <td className="whitespace-nowrap px-4 py-4 text-slate-900 font-bold">₹{order.totalAmount.toFixed(2)}</td>
                      <td className="whitespace-nowrap px-4 py-4 text-slate-700 font-semibold">{order.paymentMethod === 'Cash' ? 'Cash' : 'Online / UPI'}</td>
                      <td className="whitespace-nowrap px-4 py-4 text-slate-700 font-semibold text-xs">{order.dateTime}</td>
                      <td className="whitespace-nowrap px-4 py-4 text-slate-700">
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            className="inline-flex items-center justify-center rounded-lg border border-blue-200 bg-blue-50 p-2.5 text-blue-600 transition hover:bg-blue-100 hover:border-blue-300"
                          >
                          ...
                          <button
                            ...
                            title="Delete order"
                            className="inline-flex items-center justify-center rounded-lg border border-red-200 bg-red-50 p-2.5 text-red-600 transition hover:bg-red-100 hover:border-red-300"
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </div>
